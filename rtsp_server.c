...
rtsp_demo_handle rtsp_new_demo (int *_port)
{
	...
	opt = 1;
	ret = setsockopt(sockfd, SOL_SOCKET, MSG_NOSIGNAL, (const void *)&opt, sizeof(opt));
	if (ret == SOCKET_ERROR) {
		warn("setsockopt MSG_NOSIGNAL failed: %s\n", sk_strerror(sk_errno()));
	} 
	if (port <= 0)
		port = 554;
	#if 1
...
{
	if(codecBuf == NULL)return;
	if(codecID == RTSP_CODEC_ID_VIDEO_H264){
		strncpy(codecBuf,"H264",buflen);
	}else if(codecID == RTSP_CODEC_ID_VIDEO_H265){
		strncpy(codecBuf,"H265",buflen);
	}else if(codecID == RTSP_CODEC_ID_VIDEO_MPEG4){
		strncpy(codecBuf,"MPEG4",buflen);
	}else if(codecID == RTSP_CODEC_ID_AUDIO_G711A){
		strncpy(codecBuf,"G711A",buflen);
	}else if(codecID == RTSP_CODEC_ID_AUDIO_G711A){
		strncpy(codecBuf,"G711A",buflen);
	}else if(codecID == RTSP_CODEC_ID_AUDIO_G711U){
		strncpy(codecBuf,"G711U",buflen);
	}else if(codecID == RTSP_CODEC_ID_AUDIO_G726){
		strncpy(codecBuf,"G726",buflen);
	}else if(codecID == RTSP_CODEC_ID_AUDIO_AAC){
		strncpy(codecBuf,"AAC",buflen);
	}else{
		strncpy(codecBuf,"null",buflen);
	}
}
...
int rtsp_do_event (rtsp_demo_handle demo)
{
	...
	while (cc) {//����rtsp_client_connection
